...
echo "$($PSQL "SELECT DISTINCT name FROM teams FULL JOIN games ON winner_id=team_id OR opponent_id=team_id WHERE round='Eighth-Final' AND year=2014")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT name FROM teams FULL JOIN games ON team_id=winner_id GROUP BY name HAVING COUNT(winner_id)>0 ORDER BY name")"
